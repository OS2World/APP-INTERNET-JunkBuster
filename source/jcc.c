...
char DEFAULT_USER_AGENT[] ="User-Agent: Mozilla/5.0 (OS/2; U; Warp 4.5;)";
...
char *blockfile    = NULL;
char *blockimgfile = NULL; /* TINYGIF (Kiewitz) */
...
chat(struct client_state *csp)
{
	...
	|| (p =     trust_url(http, csp))) {
        ...
        /* Now all URLs that got denied will get logged as well (Kiewitz) */

        if(DEBUG(GPC)) {
           fprintf(logfp, "%s: DENY %s%s\n",
           csp->ip_addr_str, http->hostport, http->path);
         }

         /* TINYGIF (Kiewitz) - Now use block_imageurl */
         if (tinygif > 0) {
            if ( block_imageurl( http,csp ) ) {
               if(DEBUG(GPC)) {
                  fprintf(logfp, "%s: IDENY %s%s\n",
                  csp->ip_addr_str, http->hostport, http->path);
                }
               if (tinygif == 1)
         ...
         /* End of modification TINYGIF (Kiewitz) */
	...
	/* Proxy-Client IP will get shown in log and on screen (Kiewitz) */

	if(DEBUG(GPC)) {
		fprintf(logfp, "%s: %s%s\n",
			csp->ip_addr_str, http->hostport, http->path);

		if (url_checkpic(http->path)==0) {
		   /* Don't log Image-URLs to Log-File (Kiewitz) */
...
/* New Function - Checks if URL has .gif or .jpg extension (Kiewitz) */
int
url_checkpic(char *str) 
{
   char *helper;
   helper = strrchr(str, 46);
   if (helper==NULL) {
      return (0);
    } else {
	   if (strcmp(helper, ".gif")==0) return (1);
	   if (strcmp(helper, ".jpg")==0) return (1);
	   if (strcmp(helper, ".jpeg")==0) return (1);
	  return (0); }
}
...
main(int argc, char *argv[])
{
	...
	default_configfile = "config\\junkbstr.ini";
			...
			*q = '\0';
			...
			if(strcmp(cmd, "blockfile") == 0) {
				...
				continue;
                        ...
                        /* TINYGIF (Kiewitz) */
			if(strcmp(cmd, "blockimgfile") == 0) {
				blockimgfile = strdup(arg);
				continue;
			}
	...
	if(blockfile)    add_loader(load_blockfile);
        ...
        /* TINYGIF (Kiewitz) */
	if(blockimgfile) add_loader(load_blockimgfile);
	...
	end_proxy_args();
        ...
        /* Shows that Proxy is really up and running (Kiewitz) */
        fprintf(logfp, "JunkBuster/2 is now running...\n");
